...
describe('InterviewsService', () => {
  ...
  beforeEach(() => {
    ...
    service = new InterviewsService(interviewsRepository, opportunitiesRepository);
  });
  ...
  it('returns interviews for an owned opportunity', async () => {
    ...
    interviewsRepository.findAllForOpportunity.mockResolvedValueOnce(interviews as never);

    await expect(service.findAllForOpportunity('user-1', 'opportunity-1')).resolves.toEqual(interviews);
  });

  it('creates an interview for an owned opportunity', async () => {
    const dto = { type: 'Technical', scheduledAt: new Date('2026-09-10T10:00:00.000Z') };
    const interview = { id: 'interview-1', ...dto };
    opportunitiesRepository.findOne.mockResolvedValueOnce({ id: 'opportunity-1' } as never);
    interviewsRepository.create.mockResolvedValueOnce(interview as never);

    await expect(service.create('user-1', 'opportunity-1', dto)).resolves.toEqual(interview);
  });
  ...
  it('rejects deleting a missing interview', async () => {
    ...
    await expect(service.remove('user-1', 'opportunity-1', 'interview-1')).rejects.toThrow(
      NotFoundException,
    );
  });
});
