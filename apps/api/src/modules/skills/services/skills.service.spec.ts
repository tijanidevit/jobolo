...
describe('SkillsService', () => {
  it('calculates demand and proficiency gaps from owned opportunities', async () => {
    ...
    skillsRepository.findAllForUser.mockResolvedValue([{ id: 'skill-1', skill: 'TypeScript', proficiency: 60 }] as never);
    ...
    const service = new SkillsService(skillsRepository, opportunitiesRepository);
    const result = await service.getIntelligence('user-1');

    expect(result.analyzedOpportunities).toBe(2);
    expect(result.skills.find((skill) => skill.skill === 'TypeScript')).toMatchObject({
      demandCount: 2,
      demandPercentage: 100,
      proficiency: 60,
      gap: 40,
    });
    expect(result.skills.find((skill) => skill.skill === 'React')).toMatchObject({ demandCount: 1 });
  });
});
