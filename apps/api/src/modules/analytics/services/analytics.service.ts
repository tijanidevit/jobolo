...
export class AnalyticsService {
  ...
  async getOverview(userId: string) {
    ...
    for (const opportunity of opportunities) {
      statusCounts.set(
        opportunity.stage,
        (statusCounts.get(opportunity.stage) ?? 0) + 1,
      );
    ...
    const transitions = activities
      ...
      .filter(
        (transition): transition is StageTransition => transition !== null,
      );
    ...
    const applicationDates = new Map(
      applications.map((opportunity) => [
        opportunity.id,
        new Date(opportunity.dateApplied!).getTime(),
      ]),
    );
    const responseTimes = transitions
      .filter(
        (transition) =>
          ...
          applicationDates.has(transition.opportunityId),
      )
      .map(
        (transition) =>
          (transition.occurredAt.getTime() -
            applicationDates.get(transition.opportunityId)!) /
          86_400_000,
      )
      .filter((days) => days >= 0);
    const stageTimes = transitions
      .map((transition, index) => {
        const previous = transitions
          .slice(0, index)
          .reverse()
          .find((item) => item.opportunityId === transition.opportunityId);
        return previous
          ? (transition.occurredAt.getTime() - previous.occurredAt.getTime()) /
              86_400_000
          : null;
      })
    ...
    const responseCount = applications.filter((opportunity) =>
      transitions.some(
        (transition) =>
    ...
    const offeredCount = opportunities.filter((opportunity) =>
      OFFER_STAGES.has(opportunity.stage),
    ).length;
    const salaryValue = (opportunity: Opportunity) => {
      const min =
        opportunity.salaryRangeMin == null
          ? null
          : Number(opportunity.salaryRangeMin);
      const max =
        opportunity.salaryRangeMax == null
          ? null
          : Number(opportunity.salaryRangeMax);
    ...
    const currencies = new Set(
      opportunities
        .filter((opportunity) => salaryValue(opportunity) !== null)
    ...
    return {
      ...
      advanced: {
        responseRate:
          applicationsCount > 0
            ? Number(((responseCount / applicationsCount) * 100).toFixed(1))
            : null,
        interviewConversionRate:
          applicationsCount > 0
            ? Number(((interviewedCount / applicationsCount) * 100).toFixed(1))
            : null,
        offerConversionRate:
          applicationsCount > 0
            ? Number(((offeredCount / applicationsCount) * 100).toFixed(1))
            : null,
