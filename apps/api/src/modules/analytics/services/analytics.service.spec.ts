...
describe('AnalyticsService', () => {
  it('returns metrics and sorted distributions for the user', async () => {
    ...
    const service = new AnalyticsService(opportunitiesRepository, interviewsRepository, activitiesRepository);
  ...
  it('calculates conversion, response timing, and salary analytics', async () => {
    ...
    opportunitiesRepository.find.mockResolvedValue([
      {
        id: 'opportunity-1', stage: 'interview', dateApplied: appliedAt, companyCountry: 'Nigeria',
        jobTitle: 'Backend Engineer', source: 'Referral', companySize: 'Startup',
        salaryRangeMin: 80000, salaryRangeMax: 100000, currency: 'USD',
      },
      {
        id: 'opportunity-2', stage: 'applied', dateApplied: appliedAt, companyCountry: 'Nigeria',
        jobTitle: 'Frontend Engineer', source: 'LinkedIn', companySize: 'Enterprise',
        salaryRangeMin: 60000, salaryRangeMax: 60000, currency: 'USD',
      },
    ] as Opportunity[]);
    interviewsRepository.find.mockResolvedValue([{ id: 'interview-1', opportunityId: 'opportunity-1' }] as Interview[]);
    activitiesRepository.find.mockResolvedValue([
      { opportunityId: 'opportunity-1', type: 'status_change', description: 'Stage changed from applied to interview', occurredAt: responseAt },
    ] as Activity[]);
    ...
    expect(result.advanced.successBySource.find((row) => row.label === 'Referral')).toMatchObject({ responseRate: 100 });
  });
});
