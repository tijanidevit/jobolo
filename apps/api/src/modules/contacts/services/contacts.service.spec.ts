...
describe('ContactsService', () => {
  ...
  it('returns all contacts for an owned opportunity', async () => {
    ...
    contactsRepository.findAllForOpportunity.mockResolvedValueOnce(
      contacts as never,
    );
  ...
  it('updates a contact owned by the user', async () => {
    ...
    await expect(
      service.update('user-1', 'opportunity-1', 'contact-1', {
        name: 'Updated name',
      }),
  ...
  it('rejects deleting a contact that does not belong to the opportunity', async () => {
    ...
    await expect(
      service.remove('user-1', 'opportunity-1', 'contact-1'),
    ).rejects.toThrow(NotFoundException);
  });
});
