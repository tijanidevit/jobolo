...
export const opportunityFormSchema = z.object({
  ...
  jobUrl: z.string().refine((value) => !value || /^https?:\/\//.test(value), 'Enter a valid URL starting with http:// or https://'),
  employmentType: z.enum(['', 'full_time', 'part_time', 'contract', 'freelance', 'internship']),
  workArrangement: z.enum(['', 'remote', 'hybrid', 'onsite']),
  stage: z.enum([
    'discovered', 'interested', 'applied', 'recruiter_contact', 'screening', 'interview', 'final_round',
    'offer', 'accepted', 'declined', 'rejected', 'withdrawn', 'ghosted', 'expired',
  ]),
