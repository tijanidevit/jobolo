...
export function OpportunityKanban() {
  ...
  if (error)
    return (
      <ErrorState
        message={getErrorMessage(error, 'We could not load your pipeline.')}
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm text-slate-500">
            Drag cards between stages or use the stage selector on each card.
          </p>
          <p className="mt-1 text-xs text-slate-400">
            {opportunities.length} opportunities in your pipeline
          </p>
        </div>
        {isChangingStage && (
          <span className="text-xs font-medium text-blue-600">Saving stage change...</span>
        )}
      </div>
        ...
        <div className="flex min-h-[32rem] gap-4">
          {OPPORTUNITY_STAGES.map((stage) => {
            ...
            return (
              <section
                key={stage.value}
                aria-label={`${stage.label} opportunities`}
                onDragOver={(event) => {
                  event.preventDefault();
                  setDropStage(stage.value);
                }}
                onDragLeave={() =>
                  setDropStage((current) => (current === stage.value ? null : current))
                }
                onDrop={(event) => {
                  event.preventDefault();
                  void handleDrop(stage.value);
                }}
                className={`flex w-72 shrink-0 flex-col rounded-xl border p-3 transition-colors ${isDropTarget ? 'border-blue-400 bg-blue-50/70' : 'border-slate-200 bg-slate-50/80'}`}
              >
                ...
                <div className="flex flex-1 flex-col gap-3">
                  {stageOpportunities.map((opportunity) => (
                    <KanbanCard
                      ...
                      onStageChange={(nextStage) => void moveOpportunity(opportunity, nextStage)}
                    />
                  ))}
                  {stageOpportunities.length === 0 && (
                    <div className="flex min-h-24 flex-1 items-center justify-center rounded-lg border border-dashed border-slate-200 px-3 text-center text-xs text-slate-400">
...
function KanbanCard({
  ...
  return (
    <article
      ...
      onDragEnd={onDragEnd}
      className={`flex h-56 flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-blue-300 hover:shadow-md ${isDragging ? 'cursor-grabbing opacity-50' : 'cursor-grab'}`}
    >
      ...
      {opportunity.nextAction && (
        <p className="mt-3 line-clamp-2 text-xs font-medium text-blue-700">
          Next: {opportunity.nextAction}
        </p>
      )}
      <div className="mt-auto flex items-center justify-between gap-2 border-t border-slate-100 pt-3">
        <label className="sr-only" htmlFor={`kanban-stage-${opportunity.id}`}>
          Move {opportunity.jobTitle} to stage
        </label>
        <div className="min-w-0 flex-1">
          <SearchableSelect
            value={opportunity.stage}
            options={OPPORTUNITY_STAGES}
            onChange={(value) => onStageChange(value as OpportunityStatus)}
        ...
        <Link
          href={`/opportunities/${opportunity.id}`}
          className="inline-flex shrink-0 items-center text-xs font-medium text-blue-600 hover:underline"
        >
