...
function readFilters(params: URLSearchParams): OpportunityFilters {
  ...
  return {
    ...
    workArrangement:
      (params.get('workArrangement') as OpportunityFilters['workArrangement']) || undefined,
    employmentType:
      (params.get('employmentType') as OpportunityFilters['employmentType']) || undefined,
...
const filterOptions: Partial<
  Record<keyof OpportunityFilters, readonly { value: string; label: string }[]>
> = {
...
function getFilterValueLabel(key: keyof OpportunityFilters, value: string | number) {
  return (
    filterOptions[key]?.find((option) => option.value === String(value))?.label ?? String(value)
  );
}
...
export function OpportunityFiltersPanel() {
  ...
  const appliedFilters = (
    Object.entries(current) as [keyof OpportunityFilters, string | number | undefined][]
    ...
    <>
      <div className="border-y border-slate-200 py-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-slate-800">Refine your pipeline</p>
            <p className="text-xs text-slate-500">
              Search across roles, companies, stages, and compensation.
            </p>
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {appliedFilters.map(([key, value]) => (
              <button
                ...
                className="inline-flex items-center gap-1 rounded-full border border-blue-100 bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700 hover:border-blue-200 hover:bg-blue-100"
                aria-label={`Remove ${filterLabels[key]} filter`}
              >
                <span>
                  {filterLabels[key]}: {getFilterValueLabel(key, value as string | number)}
                </span>
                <X className="h-3 w-3" />
              </button>
            ))}
        ...
        <div className="fixed inset-0 z-50">
          ...
          >
            <div className="flex items-start justify-between border-b border-slate-200 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Combine filters to narrow your opportunities.
                </p>
                <Link
                  href="/opportunities"
                  className="mt-3 inline-flex items-center text-xs font-medium text-slate-500 hover:text-blue-600"
                >
            ...
            <form onSubmit={submit} className="flex flex-1 flex-col overflow-y-auto">
              ...
              <div className="mt-auto flex items-center justify-between border-t border-slate-200 px-6 py-4">
                <Link
                  href="/opportunities"
