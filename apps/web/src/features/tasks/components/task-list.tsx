  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Turn each opportunity into a clear next step.
          </p>
        </div>
      ...
      <CardContent className={isAdding || editingTask ? 'pt-5' : 'pt-0'}>
        ...
        {error && (
          <ErrorState
            message="We could not load tasks."
            onRetry={() => void refetch()}
            className="mt-2"
          />
        )}
        {!isLoading && !error && visibleTasks.length === 0 && !isAdding && !editingTask && (
          <div className="rounded-lg border border-dashed border-slate-200 px-4 py-7 text-center">
            ...
            <p className="mt-1 text-xs text-slate-500">Add the next action for this opportunity.</p>
          </div>
        )}
...
function TaskCard({
  ...
  return (
    <article
      className={`rounded-lg border p-4 ${task.status === 'completed' ? 'border-slate-200 bg-slate-50 opacity-70' : overdue ? 'border-red-200 bg-red-50/40' : 'border-slate-200 bg-slate-50/70'}`}
    >
      <div className="flex items-start gap-3">
        <button
          ...
          className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border ${task.status === 'completed' ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-300 bg-white'}`}
        >
          <Check className="h-3.5 w-3.5" />
        </button>
        <div className="min-w-0 flex-1">
          <p
            className={`text-sm font-semibold ${task.status === 'completed' ? 'text-slate-500 line-through' : 'text-slate-900'}`}
          >
            {task.title}
          </p>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
            <span
              className={`rounded-full px-2 py-1 font-medium ${task.priority === 'high' ? 'bg-red-100 text-red-700' : task.priority === 'low' ? 'bg-slate-100 text-slate-600' : 'bg-amber-100 text-amber-700'}`}
            >
              {task.priority}
            </span>
            {task.dueDate && (
